...
if (isset($_GET['meja'])) {
    ...
    // Get the member who placed the order (if any)
    $query_member = mysqli_query($kon, "SELECT tb_user.* FROM tb_user 
                                        JOIN tb_order ON tb_user.id_user = tb_order.id_user 
                                        WHERE tb_order.id_order = '$order[id_order]'");
    $selected_member = mysqli_fetch_assoc($query_member);
}

$member = mysqli_query($kon, "SELECT * FROM tb_user");
                            ...
                            <select name="member" class="form-control">
                                ...
                                <?php foreach ($member as $m): ?>
                                    <option value="<?= $m['id_user'] ?>" 
                                        <?php 
                                            // Automatically select the member who placed the order if it exists
                                            if (isset($selected_member) && $m['id_user'] == $selected_member['id_user']) {
                                                echo 'selected';
                                            } 
                                        ?>
                                    >
